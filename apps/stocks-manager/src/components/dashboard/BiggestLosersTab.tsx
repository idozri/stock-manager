...
import { getBiggestLosersStocks } from '../../lib/api/stocks';

interface BiggestLosersTabProps {
  isActive: boolean;
  currentPage: number;
...
export const BiggestLosersTab: React.FC<BiggestLosersTabProps> = ({
  isActive,
  currentPage,
  onPageChange,
  onAddToPortfolio,
  ...
  const {
    ...
    queryKey: ['biggestLosersStocks'],
    queryFn: getBiggestLosersStocks,
    ...
    return (
      <Alert severity="error" sx={{ mb: 2 }}>
        {error instanceof Error
          ? error.message
          : 'Failed to load biggest losers stocks'}
      </Alert>
    );
  }
      ...
      {paginatedData.length === 0 && !isLoading && (
        <Box textAlign="center" py={4}>
          <Typography variant="h6" color="text.secondary">
            No biggest losers stocks data available.
          </Typography>
        </Box>
      )}
    </Box>
