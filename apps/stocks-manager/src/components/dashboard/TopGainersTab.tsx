...
import { getTopGainersStocks } from '../../lib/api/stocks';

interface TopGainersTabProps {
  isActive: boolean;
  currentPage: number;
...
export const TopGainersTab: React.FC<TopGainersTabProps> = ({
  isActive,
  currentPage,
  onPageChange,
  onAddToPortfolio,
  ...
  const {
    ...
    queryKey: ['topGainersStocks'],
    queryFn: getTopGainersStocks,
    ...
    return (
      <Alert severity="error" sx={{ mb: 2 }}>
        {error instanceof Error
          ? error.message
          : 'Failed to load top gainers stocks'}
      </Alert>
    );
  }
      ...
      {paginatedData.length === 0 && !isLoading && (
        <Box textAlign="center" py={4}>
          <Typography variant="h6" color="text.secondary">
            No top gainers stocks data available.
          </Typography>
        </Box>
      )}
    </Box>
